...
SharedMethod.prototype.invoke = function(scope, args, remotingOptions, ctx, cb) {
  ...
  if (accepts) {
    for (var i = 0; i < accepts.length; i++) {
      ...
      //mix "_req" to arguments of methods dao.js (like find, update, etc...)
      //use only "filter" and "data", cause it safe (data may cause bodyParser error, but we cut _req from args in dao.js)
      //"where" arg - dangerous. It used in result SQL with multi transforming
      //"data" arg - dangerous. It may cause circular bodyParser error
      if(name === 'filter' || name === 'data') {
        if(!uarg || typeof uarg !== 'object')
          uarg = {};
        uarg._req = ctx.req;
      }
  ...
  //additional mix "_req" to arguments of methods dao.js (like find, update, etc...)
  //try add "_req" to arg "options" of method
  (function() {

    //parse method for get args names
    var STRIP_COMMENTS = /((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg
        , ARGUMENT_NAMES = /([^\s,]+)/g
    var sMethod = method.toString().replace(STRIP_COMMENTS, '');
    var aArgs = sMethod.slice(sMethod.indexOf('(')+1, sMethod.indexOf(')')).match(ARGUMENT_NAMES);
    if(aArgs === null)
      aArgs = [];

    //check if method accept options arg
    for(var i = 0, l = aArgs.length; i < l; i++) {
      //if accept
      //last arg always callback (callback always added after this block of code), and options always prev arg (before callback)
      //  so chech length of formatedArgs (now without callback, cause callback added after this block of code) and length of method args - 1 (-1, cause last arg - callback)
      if(aArgs[i] === 'options') {
        //check if not passed, for add
        if(formattedArgs.length < aArgs.length - 1) {
          formattedArgs.push({ //add options to args
            _req: ctx.req
          })
        } else {//if already passed, then update
          var options = formattedArgs[formattedArgs.length - 1];
          if(options && typeof options === 'object')
            options._req = ctx.req;
        }
        return;
      }
    }
  ...
  //mix "_req" to arguments of methods dao.js (like find, update, etc...)
  //some methods pass callback to dao without change it and doesn't pass another data
  callback._req = ctx.req;
